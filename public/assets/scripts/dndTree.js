/**
 * Created by Fathurozak on 17/06/2015.
 */

// Check the container is exist
if ($("#tree-container").length) {

    var diagramWidth = $("#tree-container").width();
    var route = $("#tree-container").attr("route");
    var token = $('meta[name="csrf-token"]').attr('content');

    treeJSON = d3.json(route)
        ...
        .get(function (error, treeData) {
            ...
            // size of the diagram
            var viewerWidth = diagramWidth;
            var viewerHeight = 616;
                ...
                if (d.children && d.children.length > 0) {
                    if (d.children.length < 3) {
                        d.children.forEach(function (children) {
                            var new_pos = [
                                {lft: children.pos == 'lft'},
                                {mid: children.pos == 'mid'},
                                {rgt: children.pos == 'rgt'}
                            ];

                            new_pos.forEach(function (pos) {
                                if (pos.lft == false) {
                                    var child_lft = {
                                        id: generateID(),
                                        create: true,
            ...
            function update(source) {
                ...
                nodeEnter.append("rect")
                    ...
                    .attr("height", 10)
                    .attr("style", "stroke:#999;stroke-width:0.5")
                    .style("fill", function (d) {
                        return d._children ? "lightsteelblue" : "#fff";
                    });
